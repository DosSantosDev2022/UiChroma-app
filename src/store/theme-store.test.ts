...
describe('useThemeStore', () => {
	beforeEach(() => {
		...
		const { setTheme, setSelectedColor, setCustomColor } =
	...
	it('should set custom color with setCustomColor', () => {
		...
		const { customColor, selectedColor, lightColors } =
			useThemeStore.getState()
	...
	it('should update light theme color', () => {
		act(() => {
			useThemeStore
				.getState()
				.updateLightColor('primary', colord('#123456').toHslString())
		})
	...
	it('should update dark theme color', () => {
		act(() => {
			useThemeStore
				.getState()
				.updateDarkColor('primary', colord('#abcdef').toHslString())
		})
