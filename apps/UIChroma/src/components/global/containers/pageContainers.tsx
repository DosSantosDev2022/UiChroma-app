...
const SectionPage = ({ children }: { children: ReactNode }) => {
  return (
    <section className="col-span-3 w-full rounded-md  border p-4 shadow-sm">
      {children}
    </section>
  )
}
...
const SectionNavigation = ({ children }: { children?: ReactNode }) => {
  return (
    <section className="sticky top-0 col-span-1 h-screen w-full space-y-2 border px-8 py-5 shadow-sm">
      {children}
    </section>
  )
}
