...
vi.mock('../../hooks/useCalendar', () => {
  return {
    useCalendar: vi.fn(() => ({
      ...
      dates: Array.from({ length: 30 }, (_, i) => new Date(2024, 7, i + 1)),
      selectedDate: null,
      startDate: null,
      endDate: null,
...
describe('Component Calendar', () => {
  it('should render the calendar component correctly', () => {
    render(
      <Calendar value={new Date(2024, 7, 1)} onChange={vi.fn()} range={false} />
    )
  ...
  it('should invert startDate and endDate if endDate is before startDate', async () => {
    ...
    render(
      <Calendar value={new Date(2024, 7, 1)} onChange={vi.fn()} range={true} />
    )
